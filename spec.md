...
The ranking direction is conditional until the field schema is inspected: if the selected residue-level field explicitly means model assignment/confidence with smaller values weaker, rank smallest first. **Predeclared cutoff:** flag every residue in 94–312 with local pLDDT < 70.0, then rank flagged residues by ascending raw pLDDT and retain shared ranks for ties. This is a caution/triage threshold, not an automatic exclusion rule; report zero passing residues explicitly if none pass. Show how many pass. If no documented residue-level field maps cleanly to the canonical sequence, report that no defensible shortlist can be produced rather than substituting an arbitrary column.
