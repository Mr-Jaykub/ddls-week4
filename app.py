...
<section id="summary" class="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200"></section>
<div class="mt-6 grid gap-6 lg:grid-cols-2"><section class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200"><div class="mb-3 flex items-center justify-between"><h2 class="text-lg font-semibold">Structure</h2><span class="text-xs text-slate-500">mmCIF · pLDDT from B-factor</span></div><div id="viewer" class="viewer overflow-hidden rounded-xl bg-slate-950"></div><div id="legend" class="mt-3 flex flex-wrap gap-3 text-xs"></div><p class="mt-3 text-sm text-slate-600"><strong>Scope:</strong> residues 18–28 are marked separately and excluded from the shortlist. Residues 293–312 and 183/185 are highlighted as distinct flagged regions.</p></section>
...
function renderChart(d){const el=document.getElementById('chart'), W=760,H=300,p={l:48,r:16,t:18,b:34};let vals=d.ranked_flagged_residues;let all=[];let raw=window.raw;for(let i=94;i<=312;i++)all.push({x:i,y:raw[i]});let min=Math.min(...all.map(x=>x.y)), max=100;let X=x=>p.l+(x-94)/(312-94)*(W-p.l-p.r),Y=y=>p.t+(max-y)/(max-min)*(H-p.t-p.b);let path=all.map((q,i)=>(i?'L':'M')+X(q.x).toFixed(1)+' '+Y(q.y).toFixed(1)).join(' ');let dots=vals.map(q=>`<circle cx="${X(q.residue)}" cy="${Y(q.plddt)}" r="4" fill="#dc2626"/>`).join('');el.innerHTML=`<svg viewBox="0 0 ${W} ${H}" class="h-full w-full" role="img" aria-label="pLDDT across residues 94 to 312"><line x1="${p.l}" x2="${W-p.r}" y1="${Y(70)}" y2="${Y(70)}" stroke="#dc2626" stroke-dasharray="6 4"/><path d="${path}" fill="none" stroke="#334155" stroke-width="2"/>${dots}<text x="${p.l+5}" y="${Y(70)-6}" font-size="12" fill="#b91c1c">70 cutoff</text><text x="${p.l}" y="${H-8}" font-size="12">94</text><text x="${W-p.r-20}" y="${H-8}" font-size="12">312</text><text x="6" y="${p.t+8}" font-size="12">100</text><text x="12" y="${Y(70)+4}" font-size="12">70</text></svg>`}
